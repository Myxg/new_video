...
def search(request):
    ...
    if request.method == 'POST':
        ...
        if content[0] == '':
            ...
            for i in listcon:
                # print(i)
                match = form_ShiPin_Report.objects.filter(Q(Single_Line_ShiPinMingCheng__icontains=i))
                list1 = match
                # for h in match:
                #     print(h.ID)
...
def list(request):
    if request.method == 'GET':
        ...
        # print(data)
        # content = data['form_ShiPin']
        match = form_ShiPin_Report.objects.filter(Q(Single_Line_ShiPinMingCheng__icontains=name))
        for i in match:
            list1.append(i)
        list2 = list1
        data = {}
